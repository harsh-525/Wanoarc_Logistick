...
def addtobagSave(stock, request, quantity):
    user = Fquestion.objects.filter(id=request.user.id).first()
    print(user)
    if user is None:
        return False
    getStock = Stock.objects.filter(id=stock.id).first()
    if getStock is None:
        return False
    getStock.quantity = int(getStock.quantity) - int(quantity)
    getStock.save()
    totalprice = float(quantity) * float(stock.price)
    pitem = Bag.objects.filter(stock_id=stock.id, c_id=user.id).first()
    if pitem is None:
        newItem = Bag.objects.create(stock_id=stock, c_id=user, quantity=quantity, total_price=totalprice)
        newItem.save()
    else:
        print("existing item...")
        pitem.quantity = int(pitem.quantity) + int(quantity)
        pitem.total_price = int(pitem.total_price) + totalprice
        pitem.save()
...
def addtobag(request):
    if validateUser(request):
        ...
        if int(quantity) <= int(stocks.quantity):
            addtobagSave(stocks, request, quantity)
...
def cbag(request):
    if validateUser(request):
        bagItems = Bag.objects.filter(c_id=request.user.id)
        print(bagItems)
        return render(request, 'customers_bag.html', {'bagItems': bagItems})

    return redirect('/clogin')


def cbagsubmit(request):
    if validateUser(request):
        if request.method == 'POST':
            orders = Bag.objects.filter(c_id=request.user.id)
            print(orders)
            # placing a new order
            user = Fquestion.objects.filter(id = request.user.id).first()
            newOrder = Order.objects.create(c_id=user)
            newOrder.save()
            for order in orders:
                newitem = OrderDetail.objects.create(order_id=newOrder, stock_id=order.stock_id, quantity=order.quantity
            ...
            messages.info("Error placing the order...")
